...
SCHEMA = """
CREATE TABLE IF NOT EXISTS agent_native_replacements (
 id TEXT PRIMARY KEY, request_id TEXT NOT NULL UNIQUE, fingerprint TEXT NOT NULL,
 predecessor_id TEXT NOT NULL UNIQUE REFERENCES agent_native_agents(id),
 successor_id TEXT NOT NULL UNIQUE REFERENCES agent_native_agents(id),
 parent_id TEXT REFERENCES agent_native_agents(id), reason TEXT NOT NULL,
 handoff TEXT NOT NULL, created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS agent_native_retirements (
 agent_id TEXT PRIMARY KEY REFERENCES agent_native_agents(id),
 evaluation_id TEXT REFERENCES agent_native_purpose_evaluations(id),
 replacement_id TEXT REFERENCES agent_native_replacements(id),
 ...
 CHECK((evaluation_id IS NOT NULL) != (replacement_id IS NOT NULL))
);
"""
...
def migrate_subtrees(conn):
    ...
    if not row or ('decision_agent_id' in row[0] and 'replacement_id' in row[0]
                   and 'evaluation_id TEXT NOT NULL UNIQUE' not in row[0]):
        return
    ...
    try:
        ...
        replacement = 'replacement_id' if 'replacement_id' in old_columns else 'NULL'
        conn.execute(
            'INSERT INTO agent_native_retirements '
            '(agent_id,evaluation_id,replacement_id,source,decision_agent_id,retired_at) '
            f'SELECT agent_id,evaluation_id,{replacement},source,{decision},retired_at '
...
def validate_subtree(conn, agent_ids, *, current_effect=None):
    """Reject a lifecycle end if any affected identity has unresolved authority."""
    from agent_native.acceptance import pending_required
    for index, current_id in enumerate(agent_ids):
        current_revision = conn.execute(
            'SELECT soul_revision FROM agent_native_agents WHERE id=?', (current_id,),
        ).fetchone()[0]
        prefix = 'Retirement descendant has' if index else 'Retirement has'
        ...
        excluded = current_effect if current_id == agent_ids[0] else None
        unresolved_effect = conn.execute(
            ...
            'AND NOT (e.run_id=? AND e.call_id=?) LIMIT 1',
            (current_id, *(excluded or ('', ''))),
        ).fetchone()
...
def retire_subtree(conn, *, agent_ids, decision_agent_id, evaluation_id=None,
                   replacement_id=None, root_source='agent'):
    """Apply a previously validated lifecycle end to an exact active subtree."""
    ...
    for index, current_id in enumerate(agent_ids):
        conn.execute(
            ...
            '(agent_id,evaluation_id,replacement_id,source,decision_agent_id,retired_at) '
            'VALUES(?,?,?,?,?,?)',
            (current_id, evaluation_id, replacement_id,
             root_source if index == 0 else 'parent', decision_agent_id, retired_at),
        )
        conn.execute(
...
def retire(conn, *, validate, agent_id, run_id, call_id, arguments):
    ...
    with write_txn(conn):
        ...
        validate_subtree(conn, agent_ids, current_effect=(run_id, call_id))
        retired_at = retire_subtree(
            conn, agent_ids=agent_ids, decision_agent_id=agent_id,
