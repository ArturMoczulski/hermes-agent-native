...
def unresolved_terminal_notices(conn, agent_id):
    """Return narrowly proven failed-status notices, rejecting other uncertainty.
    ...
    for opid,source,run_id,item,summary,text,status,url,label in rows:
        try:
            ...
            if (status != 'unknown' or source != 'terminal:'+run_id or not run or run[0] != 'failed'
                    or summary != 'Attempt failed' or text != expected_text or url is not None or label is not None):
                raise ValueError
...
def record_notices(conn, run_id, notices):
    ...
    for opid in notices:
        event(conn,run_id,'work.notification_unresolved',
              'Prior failed-attempt status comment remains unconfirmed: '+opid+'. It will not be resent; task effects are settled.')
