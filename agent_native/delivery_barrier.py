...
def unresolved_terminal_notices(conn, agent_id):
    ...
    for opid,source,run_id,item,summary,text,status,url,label in rows:
        try:
            ...
            if (source != 'terminal:'+run_id or not run or terminal_state not in ('failed','interrupted','limit_reached','retryable_failure','paused')
                    or summary != expected_summary or text != expected_text or url is not None or label is not None):
                raise ValueError
            if status == 'pending' and terminal_state in ('interrupted','limit_reached','retryable_failure','paused'):
