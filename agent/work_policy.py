...
def tool_schemas(context=None):
    ...
    parameters = {
        ...
        'child_autonomy_configure': _object({'child_id':string,
            'level':{'type':'integer','minimum':1,'maximum':5},
            'expected_revision':{'type':'integer','minimum':1},'reason':string},
        ...
        'purpose_evaluate': _object({'judgment':{'type':'string','enum':['continue','wait','clarify','retire_candidate']},
            'evidence':{'type':'array','items':{'type':'string','minLength':1,'maxLength':2000},'maxItems':100},
            'remaining_obligations':{'type':'array','items':{'type':'string','minLength':1,'maxLength':2000},'maxItems':100},
            'uncertainty':{'type':['string','null'],'maxLength':4000},
    ...
    descriptions = {
        ...
        'child_autonomy_configure': 'Configure the eagerness level of one direct child after inspecting it. Supply its exact current autonomy revision and explain why the level fits the delegated responsibility. This affects future attempts only and cannot change purpose, grants, model, cadence, owner review policy, identity or descendants.',
        ...
        'repository_file_read': 'Read one existing UTF-8 file inside this agent\'s granted project workspace. A missing file is a file-level error, not evidence that the workspace grant is absent. Returns content and a hash required for safe replacement. Symlinks and paths outside the workspace are rejected.',
        'repository_file_write': 'Create or replace one UTF-8 file inside this agent\'s granted project workspace. For a new or initially empty project, set expected_sha256 to missing. For replacement, supply the prior read hash. Concurrent changes are rejected.',
        'repository_command': 'Run one bounded non-shell development command in this agent\'s granted project workspace. Use rg --files to inspect an empty or unfamiliar project. Only approved rg, Git, Python, pytest, Node and npm commands are accepted; pwd and ls are not available.',
...
def protected_prompt(context):
    return (
        ...
        'You may tune a direct child autonomy level for future attempts after inspection; this never widens authority or overrides owner review policy. '
