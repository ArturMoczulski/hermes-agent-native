...
The deterministic question journey failed before the new overview existed, then passed
through question display, owner answer, retained history and saved output. The paired
required-review journey now proves that the decision precedes current work, opens the
exact immutable output in Compact view, accepts it there, and switches Compact → Full →
Compact without changing attempt, session, state or model-call count.

Five focused browser cases cover decision, question, cadence, pause and retirement
behavior; the one stale cadence selector was updated to the new accessible overview.
TypeScript, focused ESLint, whitespace checks and the production build pass. AN-93 is
complete in Plane.
