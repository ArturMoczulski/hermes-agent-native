...
| --- | --- | --- | --- |
| UX-01 | An empty installation; create an agent from a purpose and submit twice or retry after a delayed response. | One identity appears in Chat and Monitor. The purpose is retained, initial work is queued immediately, and the UI distinguishes queued from observed planning. | 1, 7, 15 |
...
| UX-10 | Pause an agent with running descendants from Chat or Monitor. | The scope is explicit, interruption begins immediately, and stopping is shown until confirmed. Unrelated roots continue. Actual worker/action evidence backs the final stopped display. | 8, 20 |
| UX-11 | Edit a purpose or retire a parent. | Purpose Save identifies the change and triggers interruption; retirement confirmation identifies the whole subtree. Records remain inspectable and stale answers do not revive retired work. | 6, 8, 17 |
...
| UX-14 | A cadence review chooses to wait; another agent is paused or has never worked. | The review is not a completed task. Last check-in, last work and last completion stay distinct; paused cadence and never-worked states are explicit. | 2, 18 |
| UX-15 | A worker submits an artifact that does not meet the assignment criteria. | Work shows submitted/needs revision instead of accepted. The evaluator, evidence and criteria remain linked, including later acceptance of a revised result. | 23 |
| UX-16 | Repeated attempts yield no new evidence; another agent waits legitimately for an external result. | One linked progress concern shows evidence and response for the first. Waiting alone does not label the second unsuccessful. | 22 |
