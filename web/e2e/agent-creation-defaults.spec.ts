...
test('agent creation starts bounded work with collapsed 15 minute and 50 step defaults',async({page,request})=>{
  ...
  await expect(page.getByText('15 minutes · 50 model steps',{exact:false})).toBeVisible();
  ...
  expect(agent.work.limits).toEqual({timeout_seconds:900,max_iterations:50});
...
test('advanced creation limits can override both defaults',async({page})=>{
  ...
  await expect(page.getByLabel('Maximum run time (seconds)',{exact:true})).toHaveValue('900');
