...
def next_reply(messages, purpose):
    ...
    if 'E2E_CONFLICT_RECOVERY' in purpose:
        ...
        if index == 0:
            ...
            name, arguments = operation('item.update', {'item_id':target,'name':'Freshly clarified task','expected_fingerprint':result(0)['fingerprint']})
        elif index == 2:
            name, arguments = operation('cycle.create', {'name':'Recovery cycle'})
        elif index == 3:
            ...
            assert result(3)['status'] == 'conflict' and result(3)['write_attempted'] is False
            name, arguments = 'plane_resource_inspect', {'kind':'item','resource_id':target}
        elif index == 5:
            assert result(4)['resource']['name'] == 'Freshly clarified task'
            name, arguments = operation('cycle.assign', {'item_id':target,'cycle_id':result(2)['resource']['id'],'expected_cycle_id':None,'expected_item_fingerprint':result(4)['fingerprint']})
        elif index == 6:
    ...
    if 'E2E_COMMENTS_HOLD' in purpose:
        ...
        if index == 0:
            ...
            comment = result(1)['pending'][0]
            name, arguments = 'work_comments', {'item_id':target,'review_id':comment['id'],
                'response':'I will revise the draft to address your comment.','reply':True}
        elif index == 3:
            name, arguments = 'work_comments', {'item_id':target}
        elif index == 4:
            name, arguments = 'result_record', {'item_id':target,'summary':'Reviewed incoming discussion','outcome':'discovery','evaluation':'Reply posted; no draft changes claimed.','outputs':[]}
        else:
            return {'role':'assistant','content':'Comment reviewed and replied to.'}
        return {'role':'assistant','content':None,'tool_calls':[{'id':f'writer_fixture_{index}','type':'function',
            'function':{'name':name,'arguments':json.dumps(arguments)}}]}
    if 'E2E_QUESTION_HOLD' in purpose:
        target = _planning(messages)['discovery']['id']
        if index == 0:
            name, arguments = 'work_item_select', {'item_id':target}
        elif index == 1:
            name, arguments = 'work_question', {'item_id':target,'topic':'ending','question':'What kind of ending would you like?'}
        elif index == 2:
            name, arguments = 'work_question', {'question_id':result(1)['id']}
        elif index == 3:
            name, arguments = 'output_publish', {'item_id':target,'title':'Answered draft','format':'text',
                'content':'The owner requested: '+result(2)['answer']}
        elif index == 4:
            name, arguments = 'result_record', {'item_id':target,'summary':'Draft follows the answer','outcome':'submitted',
                'evaluation':'Prepared for owner review','outputs':[{'output_id':result(3)['output_id'],'version':1}]}
        else:
            return {'role':'assistant','content':'Draft saved for review.'}
        return {'role':'assistant','content':None,'tool_calls':[{'id':f'writer_fixture_{index}','type':'function',
            'function':{'name':name,'arguments':json.dumps(arguments)}}]}
...
def handle_writer_request(handler, body, server, model_name):
    ...
    if 'E2E_QUESTION_HOLD' in system_text and phase == 2:
        key = 'E2E_QUESTION_HOLD'
    if 'E2E_COMMENTS_HOLD' in system_text and phase == 1:
        key = 'E2E_COMMENTS_HOLD'
    hold = key is not None and (reselect or 'E2E_WRITER_' in key or key in ('E2E_FEEDBACK_HOLD', 'E2E_QUESTION_HOLD', 'E2E_COMMENTS_HOLD') or phase >= 7)
    ...
    if hold:
        ...
        if not server.holds.wait(key, handler.connection):
            return True
        message = (next_reply(messages, system_text) if (reselect and phase == 7) or key in ('E2E_OUTPUT_LINK_HOLD', 'E2E_FEEDBACK_HOLD', 'E2E_QUESTION_HOLD', 'E2E_COMMENTS_HOLD') else
