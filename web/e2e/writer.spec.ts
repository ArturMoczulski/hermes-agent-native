...
test('a writer purpose uses shared output and result records without a chat prompt', async ({ page, request }) => {
  ...
  await expect(page.getByRole('region', { name: 'Needs your decision', exact: true })).toContainText('Awaiting owner review')
  ...
  await expect(page.getByRole('region', { name: 'Work results', exact: true })).toContainText('Awaiting owner review')
