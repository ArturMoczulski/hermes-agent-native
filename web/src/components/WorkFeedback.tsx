...
type Feedback = { id: string; text: string; intent: 'feedback' | 'revision_request'; status: 'pending' | 'handled'; response: string | null; applicable: boolean; output_id?: string | null; output_version?: number | null };
export function WorkFeedback({ agentId, revision, outputId, outputVersion }: { agentId: string; revision: number; outputId?: string; outputVersion?: number }) {
  ...
  const [intent, setIntent] = useState<Feedback['intent']>('feedback');
  const [rows, setRows] = useState<Feedback[]>([]);
  const [page, setPage] = useState(0);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  const receipt = useRef<{ id: string; text: string; intent: Feedback['intent'] } | null>(null);
  useEffect(() => {
    let active = true;
    const load = async () => {
  ...
  const send = async () => {
    ...
    if (!receipt.current || receipt.current.text !== text || receipt.current.intent !== intent) receipt.current = { id: crypto.randomUUID(), text, intent };
    try {
      await fetchJSON(`/api/agent-native/agents/${agentId}/feedback`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        request_id: receipt.current.id, text, intent, expected_revision: revision, ...(outputId ? { output_id: outputId, output_version: outputVersion ?? 1 } : {}) }) });
  ...
  return <section aria-label="Feedback for work" className="space-y-3 rounded-xl border p-5">
    ...
    <label className="block text-sm">Feedback type
      <select aria-label="Feedback type" className="mt-1 w-full rounded border bg-background p-2" value={intent} disabled={busy} onChange={event => setIntent(event.target.value as Feedback['intent'])}>
        <option value="feedback">General feedback</option><option value="revision_request">Request revision</option>
      </select>
    </label>
    <label className="block text-sm">{intent === 'revision_request' ? 'Revision instructions' : 'Direction for subsequent work'}
    ...
    <ul className="space-y-3">{visibleReceipts.map(row => <li key={row.id} className="rounded border p-3 text-sm">
      ...
      <p>{row.intent === 'revision_request' ? 'Revision request' : 'General feedback'} · {!row.applicable ? 'Earlier purpose — not applicable' : row.status === 'handled' ? 'Agent reports handled' : 'Pending'}</p>
