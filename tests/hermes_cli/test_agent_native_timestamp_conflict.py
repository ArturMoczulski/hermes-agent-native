...
def test_cycle_assignment_still_rejects_changed_task_content(broker):
    ...
    with pytest.raises(PlaneWriteConflict):
        s.run._effect(s.conn,s.planning,effect(s,'assign',arguments={'operation':'cycle.assign','arguments':{'item_id':item,'cycle_id':cycle,'expected_cycle_id':None,'expected_item_fingerprint':seen['fingerprint']}}))
